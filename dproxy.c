...
/*****************************************************************************/
/*
 * Sends the packet in m->orginal_buf to the configured dns servers(s)
 */
void dns_send_server_request(dns_request_t *m)
{
  struct in_addr in;
...
void dns_handle_request(dns_request_t *m)
{
  ...
  if( ptr != NULL ){
	...
	cache_name_append( m->cname, m->ip );
      ...
      debug("Duplicate query: Resending request ID: %d\n", 
	    m->message.header.id);
      dns_send_server_request( m );
    }
  }else{
